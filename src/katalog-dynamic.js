...
function renderMenuItems(menuData) {
    ...
    menuData.forEach(menu => {
        // 🎯 Gunakan gambar_url dari API response dengan error handling
        const imageUrl = menu.gambar_url || 'images/placeholder.jpg';
        
        // 🎯 SIMPLIFIED: Hanya tampilkan nama menu dan tombol selengkapnya
        menuHTML += `
            <div class="menu-item" data-kategori="${menu.kategori}" data-id="${menu.id}">
                ...
                ${menu.link_halaman ? 
                    `<p><a href="${menu.link_halaman}" class="button">Selengkapnya</a> untuk melihat lebih detail</p>` : 
                    '<p>Detail segera tersedia</p>'
                }
            </div>
        `;
    });
